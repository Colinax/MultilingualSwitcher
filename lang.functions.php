...
$allowed_langs = array('bg','ca','cs','da','de','en','es','et','fi','fr','hr','hu','it','lv','nl','no','pl','pt','ru','sv','sk','tr');
...
{
    global $database;
    if (!isset($entry)) {  // set page_code  = $page_id
...
function get_page_languages()
{
    ...
    if ($get_query->numRows() > 0) {
        while ($value = $get_query->fetchRow(MYSQLI_ASSOC)) {
            if (!$admin->page_is_visible($value)) {
                continue;
            }
...
if (!function_exists('getBaseUrl')) {
    function getBaseUrl()
    {
        ...
        if (!defined('WB_REL')) {
            define('WB_REL', $wb_rel);
        }
        if (!defined('ADMIN_REL')) {
            define('ADMIN_REL', $wb_rel.'/admin');
        }
    }
